...
For recovery changes, cover request validation, changed or missing local sources, workspace locking, corrupt state, downstream invalidation, missing Markdown/SRT restoration, and partial publication recovery. Only recover a partially published note pair when the recorded content matches existing files; preserve user edits and reject completed-note conflicts. Cover TUI retry/clear-cache/edit/quit input restoration and cancellation exit code 130.
