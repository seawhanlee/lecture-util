...
TUI 실행 실패 후에는 `retry`, `clear-cache`, `edit`, `quit`을 선택합니다. `clear-cache`(단축키 `c`)는 손상되었거나 유효하지 않은 작업공간 캐시를 삭제하고 처음부터 강제로 재시도하며, `edit`(단축키 `e`)은 입력과 프롬프트를 복원한 폼을 엽니다. `retry`는 `r`, `quit`은 `q`로도 입력할 수 있습니다.
