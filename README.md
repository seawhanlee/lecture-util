...
`Ctrl+R` 또는 `Run lecture`로 실행합니다. 초기 설정은 `Ctrl+S` 또는 `Save settings`로 저장합니다. `Tab`/`Shift+Tab`으로 이동하고, Course는 `Enter` 또는 `Space`로 펼친 뒤 방향키와 `Enter`/`Space`로 선택합니다. 여러 줄 프롬프트에서 `Enter`는 줄바꿈입니다. `Esc`는 열린 선택 목록을 먼저 닫으며, 기본 화면에서는 취소합니다.
